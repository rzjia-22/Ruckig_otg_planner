...
`otg_planner` is now a joint-space planning and execution stack for the Franka
Panda in ROS 2 Humble + Gazebo Sim.
...
The system still plans in joint space. It is not a Cartesian planner and it
does not depend on MoveIt for its runtime planning loop.
...
1. read current position, velocity, and estimated acceleration from
   `/joint_states`
...
This means the executed motion is always based on the real robot state, not a
blind open-loop timing assumption.

At each cycle, the local step is time-optimal with respect to the current state,
target, and the configured velocity / acceleration / jerk bounds.

## 5. Collision Model

### 5.1 Robot Model

The Panda is approximated as a chain of spheres placed along the simplified
kinematic chain.
...
If the remaining path enters a replan margin, the planner does not wait for a
hard failure. It switches to replanning early.
...
This is the main change that made the dynamic-obstacle behavior more stable than
the previous immediate-abort policy.
...
The current strategy is much more stable than the old demo, but it still does
not guarantee eventual success for arbitrary moving obstacles.
...
That would turn the current "hold and retry" recovery policy into a stronger
"retreat, wait, then recover" policy.
